...
%     U = []; X = []; E = []; Y = [];
    ...
    lb = 0*ones(N,1);
    ub = 220*ones(N,1);
    Aeq = [eye(dly),zeros(dly,N-dly)]; 
    beq = zeros(dly,1);
%     Aeq = [zeros(21,29), eye(21)]; beq = zeros(21,1);
%     cycletimes = [];
    A_ss = [eye(2)-A -B; C 0]; 
    Q_ss = 10*eye(3);
    A_ss_ineq = [0 0 1; 0 0 -1]; 
    b_ss_ineq = [ub(1);-lb(1)];
save('mpcdat_robust.mat','P','q','Aineq','bineq','lb','ub',...
    'A','B','C','M','G','Qbar','Rbar','N','rall','Aeq','beq',...
    'dly','A_ss','Q_ss','A_ss_ineq','b_ss_ineq','W','W0','K');
end
